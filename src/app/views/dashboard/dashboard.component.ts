...
export class DashboardComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal, private socketSrv:SocketService,
  ...
  viewProcessModal(processId, processName, siteId, appId) {
    ...
    if(this.token)
      this.socketSrv.continueSend('getProcessInfo',{
        siteId:siteId,
        applicationId:appId,
        processId:processId
      },
  ...
  buildTableProcess( index, applicationName, applicationState, applicationTotalProcessCount )
  {
  ...
  format(d) {
    ...
    // this.store.dispatch(new SettingsActions.Update(appSettings));
    setTimeout(()=>{
      this.buildTableProcess( this.lastRowIndex, d.applicationName, d.applicationState, d.applicationTotalProcessCount );
    }, 
    300);
    var timeline = '<div class="card"><div class="card-header"><h3 class="card-title">List Process of '+d.applicationName+'</h3>'+
								'</div><div class="card-body">'+
									'<table class="table table-bordered table-triped" id="datatable_table_process_'+this.lastRowIndex+'"></table>'+
								'</div>'+
							'</div>';
  ...
  ngOnInit() {
    this.store.select('appSettings').subscribe(settings => {
      ...
      if(this.appSettings===undefined || (this.appSettings!=undefined && settings.siteId!=this.appSettings.siteId)){
      ...
      if( this.appSettings!=undefined && (settings.siteUpdated==true) ){
        ...
        if( this.appSettings!=undefined && (settings.applicationUpdated==true) ){
          ...
          if(this.appSettings!=undefined && settings.processUpdated==true){
            this.appSettings = settings;
            this.updateTableIndex();
          }
          else{
            if(this.appSettings!=undefined && settings.processInfoUpdated==true){
    ...
    this.dtOptions = {
      ...
      "columns": [ 
        ...
        { "orderable": false, "className": "text-center details-control", "title": "App Id", "width": "2%", "data":"applicationId" },
        { "orderable": false, "className": "text-center details-control", "title": "App Name", "width": "30%", "data":"applicationName", "name":"applicationName" },
        { "orderable": false, "className": "text-center details-control", "title": "State", "data":"applicationState" , "name":"applicationState"
          ,render: function ( data, type, row ) {
            ...
            var html = '<span class="badge badge-'+color_code[data]+'">'+data+'</span>';
            return html;
          }
        },
        { "orderable": false, "className": "text-center","title": "Total Process", "data":"applicationTotalProcessCount" , "name":"applicationTotalProcessCount"  },
        ...
        //   }
        // },
        { "orderable": false, "title": "Last Update", "data":"info" , "name":"info",
    ...
    window['angularComponentReference'] = { 
      component: this, 
      zone: this.ngZone, 
      viewProcessModal: (processId, processName, siteId, appId) => this.viewProcessModal(processId, processName, siteId, appId) };  
  ...
  updateTableIndex() {
    ...
    if ( $.fn.dataTable.isDataTable("#datatable_table_process_"+this.lastRowIndex) ) {
      console.log("ALREADY EXIST");
      $("#datatable_table_process_"+this.lastRowIndex).DataTable().clear().rows.add(this.appSettings.processlist).draw();
    }
    else{
      ...
      $("#datatable_table_process_"+this.lastRowIndex)
        ...
        "columns": [ 
          { "orderable": false, "className": "text-left", "title": "Process Id", "width": "50%", "data":"processId", "name":"processId" },
          { "orderable": false, "title": "Process Name", "width": "30%", "data":"processName", "name":"processName" },
          { "orderable": false, "className": "text-center", "title": "Process State", "data":"processState" , "name":"processState"
            ,render: function ( data, type, row ) {
              var color_code = {
                'UP': 'success',
                'DOWN': 'danger',
          ...
          { "orderable": false, "title": "Function", "data":"info" , "name":"info",
            "className": "text-center",render: function ( data, type, row ) {
              //console.log(row);
              var html = '<button class="btn btn-sm btn-primary" style="margin-left: 5px;" onclick="viewProcessModal('
                +"'"+row.processId+"',"
                +"'"+row.processName+"',"
                +"'"+siteId+"',"
                +"'"+appId+"'"
  ...
  registerClient() {
    this.loading = true;
    this.socketSrv.initSend('RegisterClient',{},null).then().catch(err=> console.log(err));
  }
  ...
  getApplicationList(siteId) {
    ...
    if(this.token && siteId.length>0 ){
      ...
      this.socketSrv.continueSend('getApplicationList',{siteId:siteId},this.token);
    }
  }
  ...
  getProcessList(applicationId) {
    ...
    if(this.token){
      ...
      this.socketSrv.continueSend('getProcessList',{
  ...
  sendAction(action){
    ...
    this.socketSrv.continueSend('doAction',
      {
